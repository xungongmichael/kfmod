...
if __name__=='__main__':
	mY = np.ascontiguousarray(pd.read_csv(
		'/home/michael/Documents/Options/SRC/PyKalman/test/Vol_SPX_18.csv').values.T)
	m = 18
	r = 3

	par  = pd.read_csv(
		'/home/michael/Documents/Options/SRC/PyKalman/test/Par_SPX_18_opt.csv',header=None)
	pars = par.values.reshape(78)
	mH, mT, mQ, vD, mZ = kl.TransformParBack18(pars)
	vC = np.zeros([m,1])
	mZ_cons = np.zeros_like(mZ)
	mZ_cons[[6,8,9,11],:] = 1
	vC_cons = np.ones([m,1])
	KF = KalmanFilter(mY, vC=vC, mZ=mZ, mH=mH, vD=vD, mT=mT, mQ=mQ, n_r=r)
	KF.set_constraint(mZ_cons = mZ_cons,vC_cons = vC_cons,diag_mH=True)
	...
	# fixD = KF._vD_ma.sum()
	# fixT = KF._mT_ma.sum()
	# fixL = KF._mQ_L_ma.sum()
	# fixX = KF._mQ_D_ma.sum()
	# nC = m*1 - fixC + 0 
	# nZ = m*r - fixZ + nC
	# nH = m*m - fixH + nZ
	# nD = r*1 - fixD + nH
	...
	# m = 18
	# N = 1000
	# vC,mZ,mH,vD,mT,mQ,mY,mX = utils.GenrSam(r,m,N,ret='twist',twist_level=0.5)
	# vC_cons = np.ones([m,1])
	# mZ_cons = np.zeros_like(mZ)
	# mZ_cons[0:r,0:r] = 1
	# KF = KalmanFilter(mY, vC=vC, mZ=mZ, mH=mH, vD=vD, mT=mT, mQ=mQ, n_r=r)
	...
	# KF.set_constraint(mZ_cons = mZ_cons,vC_cons = vC_cons,diag_mH=True)
	# t0=time.time()
	# KF.estimate(module='C++')
	# t2 = time.time()-t0
	# print 'Scipy:', t1 
	# print 'C++:',t2

	# KF.estimate(module='C++',MaxIter=5000)
	# vC_cons = np.ones([m,1])
	# mZ_cons = np.zeros_like(mZ)
	# mZ_cons[0:r,0:r] = 1
	# print KF.filter_mean


	# KF.set_constraint(mZ_cons = mZ_cons,diag_mH=True)
	# KF.forecast_obs_1h()
	#KF._disassembler()
	#KF._moment()
	#KF.estimate(MaxIter=10,steady=0,epsilon=0.01,gtol=0.001)

	# for i in range(0,50):
	# 	data = mY[:,i:i+1000].copy()
	# 	KF.Update_mY(data)
	# 	KF.estimate(module='C++', MaxIter = 50000,factr = 1E3, iPrint=1)
	# 	KF.print_parameter('vD')
